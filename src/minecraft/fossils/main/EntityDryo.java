...
public class EntityDryo extends EntityDinosaurce{
	...
	private boolean FindWheats(int range){
		if (!isSelfSitting()){
			...
			List searchlist=worldObj.getEntitiesWithinAABB(EntityItem.class, AxisAlignedBB.getAABBPool().getAABB(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(range, 4D, range));
	...
	private TileEntity GetNearestTileEntity(int targetType,int range){
		...
		public EntityAnimal spawnBabyAnimal(EntityAnimal entityanimal) {
			...
			return new EntityDryo(worldObj);
		}
