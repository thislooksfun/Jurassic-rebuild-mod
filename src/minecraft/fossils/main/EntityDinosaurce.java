...
public abstract class EntityDinosaurce extends EntityTameable implements IEntityAdditionalSpawnData{
	...
	public void increaseDinoAge(){
		...
		this.boundingBox.expand(this.getDinoAge(), this.getDinoAge(), this.getDinoAge());
	}
		...
		if (this.getDinoAge()>4){
			...
			if (this.BreedTick==0){
				...
				if (new Random().nextInt(100)<GroupAmount){
					...
					if(	worldObj.checkBlockCollision(entityegg.getBoundingBox()) && worldObj.getCollidingBoundingBoxes(entityegg, entityegg.boundingBox).size() == 0) worldObj.spawnEntityInWorld(entityegg);
	...
	public void SendStatusMessage(EnumSituation ST, EnumDinoType target) {
		...
		switch (ST){
			...
			fossils.main.mod_Fossil.ShowMessage(result,(EntityPlayer) this.getOwner());
	}
	...
	public void PediaTextCorrection(EnumDinoType Caller,EntityPlayer checker) {
		...
		fossils.main.mod_Fossil.ShowMessage(SelfName,checker);
	...
	protected boolean modelizedDrop(){
		if (!this.isModelized()) return false;
			...
			if (!worldObj.isRemote){
				this.entityDropItem(new ItemStack(fossils.main.mod_Fossil.biofossil, 1),0.0f);
	...
	protected boolean EOCInteract(ItemStack itemstack,EntityPlayer entityplayer){
		if (itemstack!=null && itemstack.itemID==Item.porkRaw.itemID){
    ...
    public boolean func_82171_bF()
    {
        ...
        return var1 != null && var1.itemID == fossils.main.mod_Fossil.Whip.itemID;
    }
