...
public class EntityDinoEgg extends Entity implements IEntityAdditionalSpawnData{
	...
	public void onUpdate() {
		...
		for (int k1 = 0; k1 < 4; k1++) {
			...
			if (worldObj.getBlockId(l1, i2, j2) == Block.snow.blockID) {
				worldObj.setBlockMetadataWithNotify(l1, i2, j2, 0, j2);
			}
		}
	...
	private void HandleHatching() {
		...
		if (this.BirthTick <= -HatchingNeedTime) {
			...
			fossils.main.mod_Fossil.ShowMessage(MsgHeadTmp+EntityDinosaurce.GetNameByEnum(DinoInside,false)+TailTmp,PlayerNearby);				
		...
		if (this.BirthTick >= HatchingNeedTime) {
			...
			switch (DinoInside) {
				...
				case Brachiosaurus:
					...
					break;
				default:
					fossils.main.mod_Fossil.ShowMessage("Bug:Impossible result.",PlayerNearby);
					this.setDead();
					return;
			}
			...
			if (worldObj.checkBlockCollision(entityliving.boundingBox) && worldObj.getCollidingBoundingBoxes(entityliving, entityliving.boundingBox).size() == 0 && (!worldObj.isAnyLiquid(entityliving.boundingBox) ^ (DinoInside == EnumDinoType.Mosasaurus))) {
				if (!worldObj.isRemote)worldObj.spawnEntityInWorld(entityliving);
				if (PlayerNearby != null) {
					fossils.main.mod_Fossil.ShowMessage(mod_Fossil.GetLangTextByKey(HEAD+"Hatched"),PlayerNearby);
				}
				this.setDead();
			} else {
				fossils.main.mod_Fossil.ShowMessage(mod_Fossil.GetLangTextByKey(HEAD+"NoSpace"),PlayerNearby);
				this.BirthTick -= 500;
			}
		}
	}
	...
	public boolean interact(EntityPlayer entityplayer) {
		...
		if (itemstack == null) {
			ItemStack shell = new ItemStack(fossils.main.mod_Fossil.Ancientegg, 1, EnumToInt(this.DinoInside));
		...
		if (FMLCommonHandler.instance().getSide().isClient()){
			if (itemstack.getItem().itemID == fossils.main.mod_Fossil.DinoPedia.itemID) {
	...
	private void showpedia(EntityPlayer checker) {
		...
		fossils.main.mod_Fossil.ShowMessage(self,checker);
		if (this.DinoInside == EnumDinoType.Mosasaurus) {
			if (this.BirthTick >= 0) {
		...
		fossils.main.mod_Fossil.ShowMessage(new StringBuilder().append(StatusText).append(TempStatus).toString(),checker);			
		fossils.main.mod_Fossil.ShowMessage(new StringBuilder().append(HatchingText).append(Progress).append("/100").toString(),checker);
