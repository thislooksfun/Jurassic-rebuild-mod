...
import fossils.entity.ai.DinoAIFishing;
...
import fossils.main.IWaterDino;
...
public class EntityPlesiosaur extends EntityDinosaurce implements IWaterDino{
	...
	protected void updateEntityActionState()
    {
		...
		if (this.riddenByEntity==null){
	        ...
	        {
	            this.dataWatcher.updateObject(18, Integer.valueOf(this.getMaxHealth()));
	        }
		}
    }
	...
	public boolean interact(EntityPlayer entityplayer)
    {
		...
		if (itemstack!=null && itemstack.itemID==Item.bowlSoup.itemID){
			...
			if (NearBy!=null){
				...
				{	
					if (NearBy.get(i) instanceof EntityItem){
						...
						if (ItemInRange.getEntityItem().itemID==Item.fishRaw.itemID || ItemInRange.getEntityItem().itemID==Item.fishCooked.itemID){
							HandleEating(10);
					...
					if (this.riddenByEntity instanceof EntityPlayerSP){
						if (this.onGround || this.isInWater()){
							if (((EntityPlayerSP)this.riddenByEntity).movementInput.jump){
					            ...
					            for(; this.rotationYaw >= 180F; this.rotationYaw -= 360F) { }
								this.moveForward=((EntityPlayerSP) this.riddenByEntity).moveForward*this.getAIMoveSpeed();
						}
